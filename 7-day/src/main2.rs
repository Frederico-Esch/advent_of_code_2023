...
impl Hand {
    ...
    fn hand_type(&self) -> HandType {
        ...
        let mut jokers = cards.get(&Card::J).unwrap_or(&0i32).to_owned();
        cards.remove(&Card::J);
        for &a in cards.values().map(|v| *v).collect::<BinaryHeap<i32>>().into_sorted_vec().iter().rev() {
            if a == 5 || a + jokers == 5{
                hand_type = Some(HandType::FiveOfAKind);
                break;
            }
            else if a == 4  || a + jokers == 4{
                hand_type = Some(HandType::FourOfAKind);
                break;
            }
            else if a == 3 || a + jokers == 3 {
                if jokers + a == 3 { jokers = 0; }
            ...
            } else if a == 2 || a + jokers == 2 {
                if jokers + a == 2 { jokers = 0; }
